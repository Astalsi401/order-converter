...
1. 建立`付款代號`（無「付款方式」欄位，固定視為`信用卡`）
1. 郵遞區號取前 3 碼
1. 更改訂單成立日期格式：`%Y%m%d`
1. `電話號碼`為空白則補`****`
1. `商品總金額`=`應開立予買家之發票金額` - `運費`
1. `訂單處理費`=26，每筆訂單一次
1. 根據當前`商品總金額`計算`金流費用`（無「付款方式」欄位，固定以信用卡 2% 計算，每筆訂單一次）
1. 根據當前`商品總金額`計算`成交手續費`
1. `商品總金額`=`加總訂單總金額` / sum(`選項價格` × `購買數量`) × (`選項價格` × `購買數量`)
1. 將運費獨立列出
1. 合併商品總表欄位
1. `入庫出貨撿貨費`=`撿貨數量`×`購買數量`×7.5
1. `進貨小計`=`進貨價`×`購買數量`
...
1. 計算`運費(箱子+包材+運費)`
1. 依倉庫調整`入庫出貨撿貨費`、`訂單處理費`，如果原廠出貨、公司倉，`訂單處理費`=0, `入庫出貨撿貨費`=0
1. `運費(箱子+包材+運費)`固定為物流倉，不因倉庫被歸零
1. 如果不是第一件商品，則`訂單金額`,`運費`,`訂單處理費`為 0
1. `利潤`=sum(`訂單金額`)-sum(`進貨小計`+`成交手續費`+`金流費用`+`入庫出貨撿貨費`+`訂單處理費`+`運費(箱子+包材+運費)`)
1. `利潤百分比`=`利潤`/`訂單金額`×100
1. 如果不是第一件商品，則`利潤`,`利潤百分比`為 0
1. 空白欄位補 0 並四捨五入至小數點後 1 位：[`商品總金額`, `進貨價`, `進貨小計`, `成交手續費`, `金流費用`, `訂單金額`]
...
1. `訂單處理費`=26，每筆訂單一次
1. `金流費用`=`金流與系統處理費`
1. `成交手續費`=`成交手續費`+`預購商品服務費`+`物流隱碼服務費`+`發票處理費`
1. `商品總金額`=(`訂單金額加總` + `全站商店抵用券`) / sum(`商品售價`) × (`商品售價`)
1. 將運費獨立列出
1. 合併商品總表欄位
1. `入庫出貨撿貨費`=`撿貨數量`×`購買數量`×7.5
1. `進貨小計`=`進貨價`×`購買數量`
